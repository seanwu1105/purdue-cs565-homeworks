...
\begin{center}
  \begin{mathpar}
    \inferrule*[Right=E-WhileTrue]
    {
      ...
      {[Y\mapsto0], Y := Y + 1 \Downarrow [Y\mapsto1]}
      \\
      \inferrule*[Right=E-WhileTrue]
      {
        \inferrule*[Left=E-True]{ }
        {[Y\mapsto1], (Y \le 1) \Downarrow_B \text{true}}
        \\
        \inferrule*[Right=E-Ass]
        {
          \inferrule*[Right=E-Add]
          {
            \inferrule*[Left=E-Var]{ }
            {[Y\mapsto1], Y \Downarrow_A 1}
            \\
            \inferrule*[Right=E-Const]{ }
            {[Y\mapsto1], 1 \Downarrow_A 1}
          }
          {[Y\mapsto1], Y := Y + 1 \Downarrow_A 2}
        }
        {[Y\mapsto1], Y := Y + 1 \Downarrow [Y\mapsto2]}
        \\
        \inferrule*[Right=E-WhileFalse]
        {\inferrule*[Left=E-False]{ }{[Y\mapsto2], Y \le 1 \Downarrow_B \text{false}}}
        {[Y\mapsto2], \textbf{while}~ (Y \le 1) ~\textbf{do}~ Y := Y + 1 ~\textbf{end} \Downarrow [Y\mapsto2]}
      }
      {[Y\mapsto1], \textbf{while}~ (Y \le 1) ~\textbf{do}~ Y := Y + 1 ~\textbf{end} \Downarrow [Y\mapsto2]}
    }
    {[Y\mapsto0], \textbf{while}~ (Y \le 1) ~\textbf{do}~ Y := Y + 1 ~\textbf{end} \Downarrow [Y\mapsto2]}
  \end{mathpar}
\end{center}
